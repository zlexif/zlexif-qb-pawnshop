...
-- Handles the actual crafting, including material consumption and item creation
RegisterServerEvent('zlexif-pawn:server:Craft', function(itemHash, materials)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    if player then
        -- Example: Remove materials from player inventory and add the crafted item
        for _, material in ipairs(materials) do
            player.Functions.RemoveItem(material.item, material.amount)
        end
        player.Functions.AddItem(itemHash, 1) -- Adjust quantity as needed
