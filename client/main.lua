    ...
    if currentStashKey then
        ...
        if stashConfig then
            if stashConfig.jobrequired and playerJob.name == Config.Job then
                ...
                SendNotification(playerServerId, 'You do not have permission to access this stash.', 'error', 5000)
            end
        else
            SendNotification(playerServerId, 'Stash configuration not found.', 'error', 5000)
        end
    end
end)
...
function Craft(craftItem)
    local function proceedWithCrafting()
        ...
        QBCore.Functions.Progressbar('Craft', Language.Progressbars.Make..Config.Crafting[craftItem].label, 15000, false, false, {
            ...
            TriggerServerEvent('zlexif-pawn:server:Craft', Config.Crafting[craftItem].hash)
            for k, v in pairs(Config.Crafting[craftItem].materials) do
                TriggerServerEvent('zlexif-pawn:Server:Removemats', v.item, v.amount)
            end
            ClearPedTasks(PlayerPedId())
        end, function() -- Cancel
            ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('zlexif-pawn:client:StartMoneyWash', function(amount)
    QBCore.Functions.TriggerCallback('zlexif-pawn:server:HasMarkedBills', function(hasBills)
        if hasBills then
